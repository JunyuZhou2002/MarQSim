...
def Trotter(input_pauli, epsilon: float, t, CNOT_matrix=None,
    ...
    # calculate the Trotter step number
    N = int(N + 1)
    print(N)
    delta_t = t/N


    # calculate the operators in each Trotter step
    for j in tqdm(range(string_num)):
        if j == 0:
            halmiton = get_halmiton(input_pauli[0])
            op = torch.matrix_exp(halmiton * (0.0 + 1.0j) * delta_t)
            res = copy.deepcopy(op)
            for op in input_pauli[0][0]:
                if op != 'I':
                    CNOT_start_and_end += 1
                if op == 'X' or op == 'Y':
                    single_q_start_and_end += 1
        else:
            halmiton = get_halmiton(input_pauli[j])
            op = torch.matrix_exp(halmiton * (0.0 + 1.0j) * delta_t)
            res = torch.matmul(op, res)
            CNOT_num += CNOT_matrix[j-1][j]
            single_q_num += single_q_matrix[j-1][j]
            if j == string_num - 1:
                for op in input_pauli[string_num - 1][0]:
                    if op != 'I':
                        CNOT_start_and_end += 1
                    if op == 'X' or op == 'Y':
                        single_q_start_and_end += 1
    ...
    trotter_step = copy.deepcopy(res)
    # N trotter step together
    for i in tqdm(range(N-1)):
        res = torch.matmul(trotter_step, res)
    ...
    a = torch.matrix_exp(op)
    output_y_0_0 = a
    print(complex_angle_3(op_1=output_y_0_0, op_2=res))

    print(CNOT_num)
    CNOT_num = CNOT_num*N
    single_q_num = single_q_num*N
    print(CNOT_num)

    return res, CNOT_num, single_q_num
...
def operation(
    ...
    print("non-zero entry for P_0:",np.count_nonzero(P_0))
    print("non-zero entry for P_1:",np.count_nonzero(P_1))
    print("non-zero entry for P_2:",np.count_nonzero(P_2))
...
def trotter_operation(
    ...
    trotter_acc, trotter_CNOT, trotter_single = [], [], []
    # output_trotter, trotter_CNOT_num, trotter_single_q_num = Trotter(input_pauli=copy.deepcopy(Kernel_list), epsilon=epsilon_list[0], t=t, CNOT_matrix=CNOT_matrix, single_q_matrix=single_q_matrix)
    # print(output_trotter)
    for epsilon in epsilon_list:
        output_trotter, trotter_CNOT_num, trotter_single_q_num = Trotter(input_pauli=copy.deepcopy(Kernel_list), epsilon=epsilon, t=t, CNOT_matrix=CNOT_matrix, single_q_matrix=single_q_matrix)

        output_trotter = output_trotter.cpu()
        output_y_0_0 = output_y_0_0.cpu()
        # print(output_trotter)
        # print(output_y_0_0)

        acc_trotter = complex_angle_3(op_1=output_y_0_0, op_2=output_trotter)
        trotter_acc.append(acc_trotter)

        trotter_CNOT.append(trotter_CNOT_num)
        trotter_single.append(trotter_single_q_num)


    return trotter_acc, trotter_CNOT, trotter_single
...
# trotter_acc, trotter_CNOT, trotter_single = trotter_operation(t=excute_time, path=args.file,
#                                                     epsilon_list=epsilon_list)
# plt.figure(figsize=(6.9, 5)) 
# plt.errorbar(trotter_acc, trotter_CNOT, fmt='-x', markersize=12)
# plt.show()
# print(trotter_acc)
# print(trotter_CNOT)
...
P_0 = np.load(args.file + '/P_0.npy')
P_1 = np.load(args.file + '/P_1.npy')
P_2 = np.load(args.file + '/P_2.npy')
print('non-zero entry for P_0:', file=output_file)
print(np.count_nonzero(P_0), file=output_file)
print('non-zero entry for P_1:', file=output_file)
print(np.count_nonzero(P_1), file=output_file)
print('non-zero entry for P_2:', file=output_file)
print(np.count_nonzero(P_2), file=output_file)
...
def lamda_get(input_pauli):
    lam = 0
    for string in input_pauli:
        lam += max(string[1], -string[1])
    return lam
input_pauli = get_pauli('_Pauli_string_' + args.file[6:])

lam = lamda_get(input_pauli)
errors = [float(item) for item in list(epsilon_list_1)]
t = excute_time

# function to calculating the total gate numbers
def data(CNOT_numses, single_q_numses, errors, t, lam):
    N_list = [int(2.0 * lam * lam * t * t / error)+1 for error in errors]
    N_num = N_list
    # append N_list, totally 20 repeatition
    for i in range(19):
        N_num = N_num + N_list
    N_numses = []
    for i in range(3):
...
# function to calculating the single qubit gate numbers
def s_data(single_q_numses, errors, t, lam):
    N_list = [int(2.0 * lam * lam * t * t / error)+1 for error in errors]
    # print(N_list)
    N_num = N_list
    for i in range(19):
    ...
    for i in range(3):
        N_numses.append(N_num)
    gate_numses = []
    for i in range(len(single_q_numses)):
        # gate = [x + y for x, y in zip(CNOT_numses[i], single_q_numses[i])]
        gate = [x + y for x, y in zip(single_q_numses[i], N_numses[i])]
        gate_numses.append(gate)
    return gate_numses
...
# CNOT_numses = data(CNOT_numses, single_q_numses, errors, t, lam)
# CNOT_numses = CNOT_numses
# CNOT_numses = single_q_numses

# processing CNOT data
acc_reses_new = []
CNOT_numses_new = []
...
plt.scatter(acc_reses_new[0], CNOT_numses_new[0], color='b', label='0.2, 0.8, 0.0', marker='x')
plt.scatter(acc_reses_new[1], CNOT_numses_new[1], color='g', label='0.8, 0.2, 0.0', marker='^')
plt.scatter(acc_reses_new[2], CNOT_numses_new[2], color='y', label='0.2, 0.4, 0.4', marker='o')
plt.legend()
plt.show()
...
# plt.show()
plt.savefig('Pauli_' + args.file[6:] + '//photo' + '.png')
...
# CNOT_reduce = (sum(y_fit1)-sum(y_fit3))/sum(y_fit1)



# # processing total gate data
# CNOT_numses = data(CNOT_numses, single_q_numses, errors, t, lam)
...
# Total_reduce = (sum(y_fit1)-sum(y_fit3))/sum(y_fit1)



# file_name = args.file + '//' + 'Reduction Result' + '.txt'
# output_file = open(file=file_name, mode="a", encoding='utf-8')

# # calculate the reduction
# print('CNOT gate reduction for MarQSim-GC-RP\n', CNOT_reduce, file=output_file)

# Total_reduce = (sum(y_fit1)-sum(y_fit3))/sum(y_fit1)
# print('Total gate reduction for MarQSim-GC-RP\n', Total_reduce, file=output_file)

# # calculate the std reduction
# reduce_std = (sum(acc_std_2)-sum(acc_std_3))/sum(acc_std_2)
# print('Standard deviation reduction:\n', reduce_std, file=output_file)
...
'''
python randomcompiler.py --file=Pauli_Na+ --num=1 --list1=1.0,0.0,0.0,0.4,0.6,0.0,0.4,0.3,0.3 --epsilon_list_1=0.1,0.067,0.05,0.04,0.033,0.0286,0.025 --excute_time=0.524 --sampling_time=20

python randomcompiler.py --file=Pauli_Na+ --num=1 --list1=0.2,0.8,0.0,0.8,0.2,0.0,0.2,0.4,0.4 --epsilon_list_1=0.1,0.067,0.05,0.04,0.033,0.0286,0.025 --excute_time=0.785 --sampling_time=20
'''
